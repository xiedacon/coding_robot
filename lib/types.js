...
types.Release = async ({ action, newType, data }, activity) => {
  switch (action) {
    case 'create':
    case 'update':
      break
...
Object.keys(types).forEach((key) => {
  exports[key] = async (activity) => {
    if (key === 'error') Object.assign(activity, { target_type: 'error', action: '', user: {} })
    if (key === 'newType') Object.assign(activity, { target_type: 'newType', action: '', user: {} })
    ...
    let { newType, data, skip } = await types[key].call(null, base, activity)
    if (skip) return
