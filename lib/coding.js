  ...
  switch (activity.line_note.noteable_type) {
    case 'MergeRequestBean':
      ...
      return dingding.markdown(
        `[MR] ${activity.line_note.noteable_title}`,
    ...
    case 'Commit':
      ...
      return dingding.markdown(
        `[Commit] ${activity.line_note.noteable_title}`,
        base + activity.line_note.noteable_url,
        [
  ...
  switch (activity.action) {
    ...
    case 'create':
      return dingding.markdown(
        ...
        [
          `@${activity.task.owner.name}`,
          `**${activity.user.name}** 评论了你的任务`,
...
exports.MergeRequestBean = async function (activity) {
  ...
  switch (activity.action) {
    ...
    case 'create':
      ...
      reviewers = reviewers || coding.developers.filter(d => d !== activity.user.name)

      return dingding.markdown(
        `[MR] ${activity.merge_request_title}`,
