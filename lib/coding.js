...
function newType (activity) {
  return dingding.markdown(
    [
      `#### **[Coding]** [发现新类型]()`,
      `@${core.manager.join(' @')}`,
      `[${activity.id}](${base}/api/project/${projectId}/activities?last_id=${parseInt(activity.id) + 1})`
    ]
  )
}
...
exports.getActivities = async function getActivities (before, lastId = 999999999) {
  if (!projectId) projectId = (await request(projectUrl)).id

  let activities = await request(`${base}t/api/project/${projectId}/activities?last_id=${lastId}&type=all&user_filter=0`)
